...
const ResetPassword = () => {
    ...
    const onclick=async(data)=>{
        try {
            const response=await axios.post("http://localhost:8000/reset/password",data,{withCredentials:true})
            alert(response.data.success_msg)
            navigate("/reset-password/verification",{state:{resetCode:response.data.resetCode}})
        } catch (error) {
            if(error.response)
            {
    ...
    <div className="flex items-center justify-center min-h-screen">
      <div className="flex items-center justify-center mx-auto  max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow-sm dark:bg-gray-800 dark:border-gray-700 w-full h-full">
        ...
        <div className="mb-5">
          ...
          >
            {isSubmitting?"Getting Confirmation Code":"Get Confirmation Code"}
          </button>
          </form>
      </div>
    </div>
