...
const email_controller = async (req, res) => {
  ...
  try {
    let result=null;
    if (req.user) {
      result = await user_model.findById(req.user.i_d);
    ...
    {
      result=await user_model.findOne({email:req.body.email})
      if(!result)
      {
        return res.status(404).send("This email is not registered in our application.")
      }
    }
    await transporter.sendMail(mailOptions);
    res.json({
      success_msg: "Verification Code Sent! Check your email.",
      resetCode: resetCode,
    });
  } catch (error) {
